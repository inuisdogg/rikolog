...
-- RLS ポリシー
DROP POLICY IF EXISTS "Authenticated users can view posts" ON board_posts;
...
DROP POLICY IF EXISTS "Users can update own posts" ON board_posts;
CREATE POLICY "Users can update own posts"
  ON board_posts FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own posts" ON board_posts;
